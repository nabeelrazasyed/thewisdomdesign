...
$(window).scroll(function () {
  ...
  $(".ordr-frm").on("click", function() {
    ...
    $('.order-form .pkg-name').html(pkgName);
    $('.order-form .pkg-name2').val(pkgName);
    $('.order-form .pkg-price').html(pkgPrice);
    $('.order-form .pkg-pri').val(pkgPrice);
    $('.order-form .pkg-plink').val(paymentLink);
    $('.order-form .package_id').attr('value', pkgId);
    $('.order-form').fadeIn();
...
$('body').on('submit', '#ordernow-form', function(e) {
  e.preventDefault();
  var contact = $(this).find('input[name="contact"]').val().replace(/[^0-9]/g, "").length;
if (contact >= 10) {
  $('.formbtn, .formbtnside, .weblpbtn').prop('disabled', true);
  $('.formbtn, .formbtnside, .weblpbtn').empty();
  $('.formbtn, .formbtnside, .weblpbtn').append('<i class="fa fa-spinner fa-spin"></i>Loading');
  // 
  var elem = $(this).serialize();
  var apiurl = "https://crm.innovatechltd.com/api/validate_new_user_order";
  var localursl = "https://localhost/crm/api/validate_new_user_order";
  $.post(apiurl, elem, function(d) {
      if (d.success == true) {
      ...
      if (d.success == false) {
          ...
          $('.formbtn, .formbtnside, .weblpbtn').prop('disabled', false);
          $('.formbtn, .formbtnside, .weblpbtn').empty();
          $('.weblpbtn').append('Get Started');
          $('.formbtnside').append('Connect With Us');
          $('.formbtn').append('Order Now');
      }
  }, 'json');
   } else {
       $('.contact_error').show();
...
//   $('.order-pg').fadeIn();
//   $('.order-pg').css('display', 'flex');
//   $('.overlay1').fadeIn();

// });

// $(".overlay1, .close-icon").on("click", function() {
//   $('.get-a-quote').fadeOut();
//   $('.overlay1').fadeOut();
